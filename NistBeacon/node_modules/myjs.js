...
function getNextRecord(fs, time, outFileName) {
	beacon.nextRecord(time, function (err, res) {
		  // prints the beacon value closest to epochTime in 128 hex characters (or 64 chars)
				...
				//	}
				//});	
				fs.appendFileSync(outFileName, str);
			});
}
...
var epochTime = //Math.floor(new Date() / 1000);
			...
			1410583756
			//1412435476	//1000kbits
			//1513707956
			;

var k;
for (k = process.argv[2]; k <= process.argv[2]; k *= 2) {
	//var outFileName = process.argv[2] + '\\input' + k + '.dat'; //windows
	var outFileName = 'input' + k + '.dat'; //linux
				//'\\input1024.dat';
	var epochFileName = 'epochTimeSave.dat';
	epochTime = parseInt(fs.readFileSync(epochFileName), 10);
	
	for (var traver = 0; traver < process.argv[3]; traver++) {
	...
	fs.writeFileSync(epochFileName, epochTime, 'utf8');
}
